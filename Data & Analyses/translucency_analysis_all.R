...
####Getting the data####
data <- read.csv("translucency_analysis_data_all.csv")
...
#85 part could detect other, 65.89%

#Getting general rate of detecting other if other could be detected
data_couldDetect %>%
  count(detectedOther) %>%
  mutate(prop = prop.table(n), per = paste0(round(100 * n/sum(n), 2), "%"))
# detectedOther  n      prop    per
#         FALSE 44 0.5176471 51.76%
#          TRUE 41 0.4823529 48.24%
...
data_couldNdetected %>%
  ...
  mutate(prop = prop.table(n), per = paste0(round(100 * n/sum(n), 2), "%"))
# translucency_change  n      prop    per
#               same 22 0.5365854 53.66%
#             switch 19 0.4634146 46.34%
...
#, , cooperation_initial_other = defection
# 
# translucency_change
# cooperation_initial same switch
#       defection      4      3
#       cooperation    2     11
# 
#, , cooperation_initial_other = cooperation
# 
# translucency_change
# cooperation_initial same switch
#       defection      1      1
...
# (Intercept)                                  0.96737    0.19923   4.856  1.2e-06 ***
# translucency_direction1                      0.21842    0.19923   1.096    0.273    
# translucency_level1                          0.13825    0.19923   0.694    0.488    
# translucency_direction1:translucency_level1 -0.05108    0.19923  -0.256    0.798  

round(exp(F.MainLM.interaction$coefficients), digits = 2)
#confidence intervals (Profile)
confint(F.MainLM.interaction)
#                                                 2.5 %    97.5 %
# (Intercept)                                  0.5875460 1.3714579
# translucency_direction1                     -0.1696993 0.6155451
# translucency_level1                         -0.2517022 0.5336803
# translucency_direction1:translucency_level1 -0.4439833 0.3414803
...
#   direction predicted
# 1 defection 0.6769231
# 2 symmetric 0.7656250
...
#   direction level predicted
# 1 defection   20% 0.6363636
# 2 symmetric   20% 0.7500000
# 3 defection   80% 0.7187500
# 4 symmetric   80% 0.7812500
...
# Coefficients:
#               Estimate Std. Error z value Pr(>|z|)   
# (Intercept)   0.9491     0.1963   4.835 1.33e-06 ***
...
mixedData <- data.frame(
  ...
  age = data$age_prolific,
  gender = data$gender_reported,
  know_name = data$know_name_game,
  experience = data$experience_econ_num,
  understood_payoffs = data$understood_payoffs
)
