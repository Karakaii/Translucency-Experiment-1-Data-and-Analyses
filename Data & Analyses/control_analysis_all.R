...
####Getting the data####
data <- read.csv("control_analysis_data_all.csv")
...
dev0 <- -2*(log((p0^k0)*((1-p0)^(N0-k0)))) #41.58883
devDiff00 <- dev0 - F.MainLM.intercept$deviance #2.603268
PRD0 <- devDiff00/dev0 #0.06259537
p00 <- pchisq(devDiff00, df=1, lower.tail=FALSE) #0.1066436
...
F.MainLM.understood <- glm(cooperation ~ understood_payoffs_factor, 
                           data = data, family = "binomial")
...
xtabs(~vision_condition+choice, data = visionData)
